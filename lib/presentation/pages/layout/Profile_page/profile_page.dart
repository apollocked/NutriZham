...
class _ProfilePageState extends State<ProfilePage> {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(context, true), style: TextButton.styleFrom(foregroundColor: const Color(0xFFEF4444)), child: Text(loc.logout)),
        ],
      ),
    );
    if (confirmed == true) {
  ...
  Widget _buildProfileHeader(AppLocalizations loc, ThemeData theme) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [theme.colorScheme.primary.withOpacity(0.08), theme.colorScheme.secondary.withOpacity(0.04)], begin: Alignment.topCenter, end: Alignment.bottomCenter),
      ...
      child: Column(children: [
        CircleAvatar(
          ...
          child: Text(_currentUser!.username[0].toUpperCase(), style: const TextStyle(fontSize: 36, color: Color(0xFF10B981), fontWeight: FontWeight.w700)),
        ),
        const SizedBox(height: 16),
        Text(_currentUser!.username, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: theme.colorScheme.onSurface)),
        ...
        Container(
          ...
          child: Text(_currentUser!.email, style: const TextStyle(fontSize: 13, color: Color(0xFF10B981), fontWeight: FontWeight.w500)),
        ),
        const SizedBox(height: 8),
        Text('${loc.age}: ${_currentUser!.age}', style: TextStyle(color: theme.colorScheme.onSurfaceVariant, fontSize: 14)),
      ]),
    );
  }

  Widget _buildStatsSection(AppLocalizations loc, ThemeData theme, FavoritesProvider favorites, MealPlannerProvider planner, List<Recipe> favoriteMeals) {
    return Padding(
      padding: const EdgeInsets.all(16),
      child: Row(children: [
        Expanded(child: StatCard(icon: Icons.favorite_outline, label: loc.favorites, value: '${favoriteMeals.length}', color: const Color(0xFFEF4444))),
        const SizedBox(width: 16),
        Expanded(child: StatCard(icon: Icons.calendar_today_outlined, label: loc.mealPlanner, value: '${planner.count}', color: const Color(0xFF3B82F6))),
      ]),
    );
  }
  ...
  Widget _buildMenuSection(AppLocalizations loc, ThemeData theme) {
    return Container(
      ...
      child: Column(children: [
        MenuItemTile(icon: Icons.favorite_rounded, title: loc.appFeature, onTap: () => context.push('/features')),
        Divider(color: theme.colorScheme.outline, height: 1, indent: 60),
        MenuItemTile(icon: Icons.settings_outlined, title: loc.settings, onTap: () => context.push('/settings')),
        Divider(color: theme.colorScheme.outline, height: 1, indent: 60),
        MenuItemTile(icon: Icons.logout, title: loc.logout, onTap: _logOutAccount, iconColor: const Color(0xFFEF4444), textColor: const Color(0xFFEF4444)),
      ]),
    );
  }

  Widget _buildFavoritesSection(AppLocalizations loc, ThemeData theme, FavoritesProvider favorites, List<Recipe> favoriteMeals) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(loc.favorites, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: theme.colorScheme.onSurface)),
        ]),
        const SizedBox(height: 16),
        if (favoriteMeals.isEmpty)
          ...
          Column(children: [
            ...favoriteMeals.take(5).map((recipe) => Padding(
              ...
              child: RecipeCard(recipe: recipe, isFavorite: true, onFavoriteToggle: () => favorites.toggleFavorite(recipe.id), onTap: () {}),
            )),
              ...
              Padding(
                ...
                child: Center(
                  child: Container(
                    ...
                    child: Text('+ ${favoriteMeals.length - 5} more', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF10B981))),
                  ),
                ),
              ),
          ]),
