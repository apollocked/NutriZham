...
class _ChangePasswordPageState extends State<ChangePasswordPage> {
  ...
  bool _showCurrentPassword = false, _showNewPassword = false, _showConfirmPassword = false;
  ...
  Future<void> _changePassword() async {
    ...
    if (!_formKey.currentState!.validate()) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(loc.pleaseFillAllFields), backgroundColor: Theme.of(context).colorScheme.error)); return; }
    if (!_isCurrentPasswordValid) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(loc.pleaseVerifyCurrentPassword), backgroundColor: Theme.of(context).colorScheme.error)); return; }
    if (_newPasswordController.text != _confirmPasswordController.text) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(loc.passwordsDoNotMatch), backgroundColor: Theme.of(context).colorScheme.error)); return; }
    setState(() => _isLoading = true);
    try {
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SectionHeader(title: loc.currentPassword, fontSize: 14, fontWeight: FontWeight.w600, barHeight: 16, padding: EdgeInsets.zero),
            ...
            SectionHeader(title: loc.newPassword, fontSize: 14, fontWeight: FontWeight.w600, barHeight: 16, padding: EdgeInsets.zero),
            const SizedBox(height: 8),
            ...
            const SizedBox(height: 24),
            SectionHeader(title: loc.confirmPassword, fontSize: 14, fontWeight: FontWeight.w600, barHeight: 16, padding: EdgeInsets.zero),
            const SizedBox(height: 8),
